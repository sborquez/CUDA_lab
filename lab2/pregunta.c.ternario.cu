...
__global__ void collision(node *grid, int n, int m)
{
    ...
    if (tId < n*m) {
        ...
        node_new = (y == 0 && (node_new & 0b00100000)) ? ((node_new & 0b11011111) | 0b10000000) :
            ((y == n-1 && (node_new & 0b10000000)) ? ((node_new & 0b01111111) | 0b00100000) :
                node_new);

        node_new = (x == 0 && (node_new & 0b01000000)) ? ((node_new & 0b10111111) | 0b00010000) :
            ((x == m-1 && (node_new & 0b00010000)) ? ((node_new & 0b11101111) | 0b01000000):
                node_new);

        grid[x + y * m] = (node_new != node_old) ? (node_new) :
            (((node_old == 0b10100000) || (node_old == 0b01010000)) ? (node_old^0b11110000) :
                node_old);
    }
}
